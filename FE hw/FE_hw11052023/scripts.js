...
//     const get_id_num = goods.filter((el) => (el.id % 2 === 0 ? el : ""));
// console.log( get_id_num);

//- 2 Посчитайте сумму всех товаров, учитывая их количество

//     const get_sum = goods.reduce((acc, elem) => acc + elem.price, 0);
// console.log( get_sum);

//- 3 Найдите значение элемента массива (продукта), цена которого будет кратна 5

    // console.log(goods.find((el)=>el.price%5===0));
    // goods.find((el)=>el.price%5===0?console.log(el.title):'' )        

//- 4 Найдите индекс элемента, count которого будет больше 11
    const get_index_elem_more11 = goods.findIndex((el) => el.count > 11);
console.log(get_index_elem_more11);
...
//- 8 Создайте новый массив, изменив только 1 сво-во count на значение 0 для всех элементов (учитывая другие 
//  сво-ва)
